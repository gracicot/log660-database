...
def login():
	...
	requestJson = json.loads(request.body.read())
	email = requestJson['identifiant']
	password = requestJson['motDePasse']
...
def recommandation():
	requestJson = json.loads(request.body.read())
	film = int(requestJson['id_film'])
	client = int(requestJson['id_client'])
